...
+------+-----------+--------------+-------------+---------------+
| book | contentId | counterparty | dataVersion | enteredDate   |
+------+-----------+--------------+-------------+---------------+
| "XY" | 5281      | "CP-1001"  | 3           | 1737654321000 |
+------+-----------+--------------+-------------+---------------+

+---------------+-----------+----+----------------+--------------+---------------+
| extTradeId    | recordComplete | id | sourceSystemId | tradeVersion | updatedDate   |
+---------------+-----------+----+----------------+--------------+---------------+
| "EXT-1001"   | true      | 2  | "SRC-1"        | 12           | 1737668121000 |
+---------------+-----------+----+----------------+--------------+---------------+
...
| book | contentId | counterparty | dataVersion | enteredDate   |
+------+-----------+--------------+-------------+---------------+
| "XY" | 5281      | "CP-1001"  | 1           | 1737654321000 |
+------+-----------+--------------+-------------+---------------+

+---------------+-----------+----+----------------+--------------+---------------+
| extTradeId    | recordComplete | id | sourceSystemId | tradeVersion | updatedDate   |
+---------------+-----------+----+----------------+--------------+---------------+
| "EXT-1001"   | true      | 2  | "SRC-1"        | 6            | 1737658921000 |
+---------------+-----------+----+----------------+--------------+---------------+
...
The full version copy pattern provides:

✅ **Complete version history** — Every version stored as an immutable snapshot  
✅ **Point-in-time queries** — O(log N) retrieval at any timestamp via the versions map  
✅ **ACID guarantees** — Transactions ensure consistency  
✅ **Associated content** — Versions `TradeStaticData` alongside `TradeBase`  

For an audit-trail pattern that stores only bin-level deltas, see [Delta Versioning Records](versioning-records-delta.md).
