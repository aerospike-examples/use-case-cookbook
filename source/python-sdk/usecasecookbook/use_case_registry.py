...
def _register() -> List[UseCase]:
    from usecasecookbook.advancedexpressions.advanced_expressions import AdvancedExpressions
    ...
    from usecasecookbook.manytomany.many_to_many_relationships import ManyToManyRelationships
    from usecasecookbook.onetomany.one_to_many_relationships import OneToManyRelationships
    from usecasecookbook.recordversioning.delta_versioning_records import DeltaVersioningRecords
    ...
    from usecasecookbook.timeseries.time_series_large_variance_demo import TimeSeriesLargeVarianceDemo
    from usecasecookbook.transactionprocessing.top_transactions_across_dcs import TopTransactionsAcrossDcs
...
def find_by_name(name: str) -> Optional[UseCase]:
...
def find_by_partial_name(partial_name: str) -> Optional[UseCase]:
    matches = find_all_by_partial_name(partial_name)
    return matches[0] if len(matches) == 1 else None


def get_by_index(index: int) -> Optional[UseCase]:
